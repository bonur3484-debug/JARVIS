...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📋 /help — список команд\n💬 Или просто напишите что угодно!",
        parse_mode="Markdown")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 *JARVIS — Команды*\n\n"
        "💬 Просто пишите любой вопрос\n"
        "🖼 Отправьте фото — опишу\n"
        "📄 Отправьте файл — прочитаю\n\n"
        "✅ *Задачи:*\n/tasks /addtask /donetask 1 /deltask 1\n\n"
        "📝 *Заметки:*\n/notes /addnote /delnote 1\n\n"
        "⏰ *Напоминания:*\n/reminders\n/remind 14:30 текст\n/delremind 1\n\n"
...
async def cmd_addtask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("❗ Пример: /addtask позвонить клиенту")
        return
...
async def cmd_donetask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("❗ Пример: /donetask 1")
...
async def cmd_deltask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("❗ Пример: /deltask 1")
...
async def cmd_addnote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = " ".join(context.args)
    if not text:
        await update.message.reply_text("❗ Пример: /addnote важная идея")
        return
    ud = get_user_data(uid)
    ud["notes"].append({"text": text, "created": datetime.now().isoformat()})
    save_user_data(uid, ud)
    await update.message.reply_text(f"📝 Сохранено: *{text}*", parse_mode="Markdown")
...
async def cmd_delnote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        save_user_data(uid, ud)
        await update.message.reply_text(f"🗑 Удалено: {removed['text']}")
    except: await update.message.reply_text("❗ Пример: /delnote 1")
...
async def cmd_remind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("❗ Пример: /remind 14:30 встреча с клиентом")
        return
    ...
    except: await update.message.reply_text("❗ Пример: /remind 09:00 позвонить")
...
async def cmd_delremind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("❗ Пример: /delremind 1")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        reply = await ask_gemini([{"role": "user", "content": caption}], image_b64=img_b64)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(file.file_path)
    caption = update.message.caption or "Прочитай и объясни этот документ."
    try:
        text_content = resp.content.decode("utf-8", errors="ignore")[:8000]
        doc_text = f"Документ '{doc.file_name}':\n\n{text_content}"
        reply = await ask_gemini([{"role": "user", "content": caption}], doc_text=doc_text)
...
def main():
    ...
    # Планировщик через job_queue (встроен в python-telegram-bot 20.x)
    app.job_queue.run_repeating(
        lambda ctx: asyncio.ensure_future(reminder_job(app)),
